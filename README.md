...
- **PDF**: `.pdf` (текст и таблицы)
  - ✅ **Автоматический OCR для отсканированных PDF**: сервис автоматически определяет отсутствие текстового слоя и применяет OCR (Tesseract) с поддержкой русского и английского языков
  - Качество распознавания: DPI 300 для максимальной точности
- **HTML**: `.html`, `.htm`
- **Изображения**: `.jpg`, `.jpeg`, `.png` (с поддержкой OCR через Tesseract)
...
- Сервис автоматически определяет PDF без текстового слоя (отсканированные документы)
- Применяется OCR с Tesseract (русский + английский языки)
- Конвертация в высоком разрешении (300 DPI) для лучшего качества распознавания
- Все системные зависимости (Tesseract, Poppler) включены в Docker-образ
